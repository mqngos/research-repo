...
% Define the period of the source
...
source.p(one_period_indices) = p0 * sin(2 * pi * source_freq * kgrid.t_array(one_period_indices));
...
% run the simulation
sensor_data_inverted = kspaceFirstOrder1D(kgrid, medium, source, sensor, input_args{:});

plot(1:10000, sensor_data);

hold on;

plot(1:10000, sensor_data_inverted);

hold off;


% additional processing of pulse inversion

summation_data = sensor_data + sensor_data_inverted;
figure
plot(1:10000, summation_data);

% take fft to check harmonic behaviors

% === Define sine wave properties ===
f0 = source_freq;          % Frequency of sine wave [Hz]
Fs = 50000000 * (10);         % Sampling frequency [Hz]
...
Y = fft(summation_data, L_pad);        % zero-padded FFT
